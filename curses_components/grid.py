...
class Help:
    ...
    def display(self):
        ...
        max_scroll = max(0, len(help_text) - (win_height - 4))  # Adjusted for border and title
        ...
        while True:
            ...
            # Clear the content area before redrawing
            for i in range(1, win_height - 1):
                win.addstr(i, 1, " " * (win_width - 2))

            text_y = 2
            for i, (key, desc) in enumerate(help_text[self.scroll_pos:]):
            ...
            key = win.getch()
            logging.debug(f"key: {key}, scroll_pos: {self.scroll_pos}, max_scroll: {max_scroll}")
            if key == ord('q'):
                break
...
class GridComponent:
    ...
    def _prepare_data(self):
        # Limit the data to max_rows. This handles both lists and iterators.
